...
- **Play counts** in D1, anonymous: every start, every "stream" (30 seconds heard, the way streaming services count a play) and every track played to the end.
- **A private stats page** at `/stats/`: visits from Cloudflare Web Analytics, plus which tracks get played, behind Cloudflare Access.
...
| `worker.js` | `playerRoutes()`: `/api/album/<slug>`, `/audio/…` and `/api/plays` |
| `stats.js` | `statsRoutes()`: the `/stats/` page and its numbers |
| `schema.sql` | The D1 `plays` table |
...
```sh
npm install github:jowtron/site-player#v0.1.0
```
...
**2. Pages.** Load `player.css` before the site's own stylesheet, so the site's `--sp-*` values win. Load `player.js` before the site's script. Put an empty `<span class="mp-slot"></span>` in the header for the tucked-away button. Then play buttons:
...
- **Plays** come straight from D1; nothing to set up beyond the binding.
